...
function extract_docx($file){
	if(file_exists($file)){
		$zip = new ZipArchive; 
		if ($zip->open($file, ZipArchive::CREATE)!==TRUE) {
			echo "Cannot open $file :( "; die;
		}
		//echo 'Inside the zip file'.'<br /><br />';
		//----------------------------Image files--------------------------
		for($a=0; $a<$zip->numFiles;$a++){
			$one = $zip->statIndex($a);
			if(dirname($one['name'])=='word/media'){
				if(strpos($newFolder,'processed')!==false){
					$newFolder = str_replace('_processed','',$newFolder);
				}
				//die($newFolder);
				if(!is_dir($newFolder)){
					mkdir($vr.'/'.$pathinfo['filename'].'_files');
				}
				if(!file_exists($newFolder.'/'.basename($one['name']))){
					if($img = $zip->getFromName('word/media/'.basename($one['name']))){
						$imgFile = $img;//One Image File
						file_put_contents(basename($one['name']),$img);
						rename(basename($one['name']),$newFolder.'/'.basename($one['name']));
					}
				}
			}
		}
		//-------------------------rels Document------------------------
		$imageExists='';
		if($rels = $zip->getFromName('word/_rels/document.xml.rels')){
			$rels = simplexml_load_string($rels);
			//$rels = simplexml_load_file($rels);
			//print_r($rels);
			foreach($rels->Relationship as $k=>$v){
				if($v->attributes()->Type =="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image"){
					//echo 'image exists 1';
					$imageExists=1;
					$imgId = $v->attributes()->Id;
					$imgTarget = $v->attributes()->Target;
					$imgName = explode('/',$imgTarget);
					$imgN["$imgId"]=$imgName[1]; 
				}
			}
			//print_r($imgN);
		}
		...
		if($imageExists==1){
			...
			foreach($imgN as $k => $v){
				$xml=str_replace('<a:blip r:embed="'.$k.'"','<img src="'.$newFolder.'/'.$v.'"',$xml);
				//echo $newFolder;	
			}
		}
		//echo $xml;
		...
		$v2=$xml;
		$zip->close();
		$v2=str_replace('<w:tbl',"<table",$v2);
		$v2=str_replace('</w:tbl>',"</table>",$v2);
		$v2=str_replace('<w:tr',"<tr",$v2);
		$v2=str_replace('</w:tr>',"</tr>",$v2);
		$v2=str_replace('w:tc',"td",$v2);
		$v2=str_replace('<w:body',"<body",$v2);
		$v2=str_replace('</w:body>',"</body>",$v2);
		$v2=str_replace('<w:p',"<p",$v2);
		$v2=str_replace('</w:p>',"</p>",$v2);
		$v2=str_replace('w:t',"span",$v2);
		$v2=str_replace('`',"",$v2);
		//die($v2);
		$v2=cleanUp($v2);
		$ct=$v2;
...
function _readFile($file){
  ...
  function _writeFile($file, $data, $returnData = false){
	...
	if($handle = fopen($file, "w+")){
	  ...
	  if(!empty($data) && fwrite($handle, $data)){
		fclose($handle);
